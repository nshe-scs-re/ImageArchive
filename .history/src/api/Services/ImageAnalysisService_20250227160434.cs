...
public class ImageAnalysisService : BackgroundService
{
    ...
    public ImageAnalysisService(ILogger<ImageAnalysisService> logger, IWebHostEnvironment env)
    {
        ...
        // Set paths relative to the solution root
        var solutionRoot = Directory.GetParent(env.ContentRootPath)?.Parent?.FullName;
        if (solutionRoot == null)
        {
            throw new DirectoryNotFoundException("Could not find solution root directory");
        }

        _modelPath = Path.Combine(solutionRoot, "predictor", "Local_data_for_model", "best_model.pth");
        _scriptPath = Path.Combine(solutionRoot, "predictor", "Local_data_for_model", "classify_image.py");
